...
bool Authenticator::auth(const string &password) {
    ...
    if (enable == 0){
        Log::log_with_date_time(password + " disabled", Log::WARN);
        return false;
    }
    return true;
}
...
void Authenticator::record(const string &password, uint64_t download, uint64_t upload) {
    ...
    if (mysql_query(&con, ("UPDATE user SET d = d + " + to_string(download) + ", u = u + " + to_string(upload) + " WHERE password = '" + password + '\'').c_str())) {
        Log::log_with_date_time(mysql_error(&con), Log::ERROR);
    }
}
